...
class HomeViewController: ParentViewController {
    ...
    fileprivate func calcProgress() {
        let calendar = Calendar.current
        let currentDate = Date()
        let dateDebut = calendar.date(byAdding: .day, value: -1, to: currentDate)!
        let dateFin = calendar.date(byAdding: .day, value: 1, to: currentDate)!
